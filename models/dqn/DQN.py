...
class DQNSolver(nn.Module):
    def __init__(self, input_size, n_actions, dropout, hidden_size) -> None:
        ...
        for ind in range(1, len(hidden_size)):
            self.fc.add_module('hidden' + str(ind),
                               nn.Linear(hidden_size[ind-1], hidden_size[ind]))
...
class DQNAgent:
    def __init__(self,  window_size=config['window_size'],
        ...
        self.env = CryptoEnv(
            df=df_btc, window_size=window_size, frame_len=frame_len)
